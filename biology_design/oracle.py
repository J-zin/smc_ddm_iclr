# Adapted from https://github.com/ChenyuWang-Monica/DRAKES/blob/master/drakes_dna/oracle.py
...
def get_gosai_oracle(mode='train', base_path=None):
    if base_path is None:
        base_path = os.environ.get('DRAKES_DATA_ROOT')
        if base_path is None:
            raise ValueError("base_path must be provided or DRAKES_DATA_ROOT env var must be set")
    if mode == 'train':
...
def get_atac_model(base_path=None):
    if base_path is None:
        base_path = os.environ.get('DRAKES_DATA_ROOT')
        if base_path is None:
            raise ValueError("base_path must be provided or DRAKES_DATA_ROOT env var must be set")
...
def cal_gosai_pred(seqs, model=None, mode='eval', base_path=None):
    """
    seqs: list of sequences (detokenized ACGT...)
    """
    if model is None:
        model = get_gosai_oracle(mode=mode, base_path=base_path)
...
def cal_gosai_pred_new(seqs, model=None, mode='eval', base_path=None):
    """
    seqs: list of sequences (detokenized ACGT...)
    """
    if model is None:
...
def cal_atac_pred(seqs, model=None, base_path=None):
    """
    seqs: list of sequences (detokenized ACGT...)
    """
    if model is None:
        model = get_atac_model(base_path=base_path)
    df_seqs = pd.DataFrame(seqs, columns=['seq'])
    pred_dataset = grelu.data.dataset.DFSeqDataset(df_seqs)
    preds = model.predict_on_dataset(pred_dataset, devices=[0])
...
def cal_atac_pred_new(seqs, model=None, base_path=None):
    """
    seqs: list of sequences (detokenized ACGT...)
    """
    if model is None:
...
def cal_gosai_emb(seqs, model=None, base_path=None):
    """
    seqs: list of sequences (detokenized ACGT...)
    """
    if model is None:
        model = get_gosai_oracle(base_path=base_path)
    df_seqs = pd.DataFrame(seqs, columns=['seq'])
    pred_dataset = grelu.data.dataset.DFSeqDataset(df_seqs)
